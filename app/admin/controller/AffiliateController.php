...
class AffiliateController extends GetUserController
{
	...
	public function useraffirecord(\think\Request $request)
	{
		...
		$total = \think\Db::name("affiliates_withdraw")->alias("a")->join("clients c", "c.id=a.uid")->leftJoin("currencies cu", "cu.id = c.currency")->leftJoin("user u", "a.admin_id=u.id")->field("a.id,a.num,a.type,a.create_time,a.status,a.reason,u.user_nickname,cu.suffix,c.username,c.companyname")->where(function (\think\db\Query $query) use($params) {
			if (!empty($params["user_nickname"])) {
				$search_desc = $params["user_nickname"];
				$query->where("u.user_nickname LIKE '%{$search_desc}%'");
			}
	...
	public function affiwithdrawrecord(\think\Request $request)
	{
		try {
			// 调试日志记录 - 记录请求开始
			$debugLog = CMF_ROOT . 'data/debug_withdraw.log';
			$logData = [
				'timestamp' => date('Y-m-d H:i:s'),
				'method' => 'affiwithdrawrecord',
				'request_params' => $this->request->param(),
				'input_data' => [
					'page' => input("page"),
					'limit' => input("limit"),
					'order' => input("order"),
				...
				'user_info' => [
					...
					'is_sale' => $this->user["is_sale"] ?? 'unknown'
				]
			];
			file_put_contents($debugLog, "=== REQUEST START ===\n" . json_encode($logData, JSON_UNESCAPED_UNICODE) . "\n", FILE_APPEND);
			...
			// 记录配置信息
			file_put_contents($debugLog, "Config - page: " . config("page") . ", limit: " . config("limit") . "\n", FILE_APPEND);
			file_put_contents($debugLog, "Actual - page: $page, limit: $limit\n", FILE_APPEND);
			...
			file_put_contents($debugLog, "Order: $order, Sort: $sort\n", FILE_APPEND);

			// 记录SQL查询构建过程
			file_put_contents($debugLog, "Building main query...\n", FILE_APPEND);
			...
			if ($this->user["id"] != 1 && $this->user["is_sale"]) {
				file_put_contents($debugLog, "Adding whereIn condition with str: " . ($this->str ?? 'null') . "\n", FILE_APPEND);
				$rows->whereIn("c.id", $this->str);
			}
			
			file_put_contents($debugLog, "Executing main query with page($page), limit($limit), order($order, $sort)...\n", FILE_APPEND);
			$rows = $rows->page($page)->limit($limit)->order($order, $sort)->select()->toArray();
			
			file_put_contents($debugLog, "Main query executed successfully. Result count: " . count($rows) . "\n", FILE_APPEND);

			// 记录count查询构建过程
			file_put_contents($debugLog, "Building count query...\n", FILE_APPEND);
			...
			file_put_contents($debugLog, "Executing count query...\n", FILE_APPEND);
			$total = $total->count();
			
			file_put_contents($debugLog, "Count query executed successfully. Total: $total\n", FILE_APPEND);

			// 记录数据处理过程
			file_put_contents($debugLog, "Processing rows data...\n", FILE_APPEND);
			...
			file_put_contents($debugLog, "Data processing completed successfully.\n", FILE_APPEND);
			file_put_contents($debugLog, "=== REQUEST SUCCESS ===\n\n", FILE_APPEND);

			return jsonrule(["status" => 200, "data" => $rows, "msg" => lang("SUCCESS MESSAGE"), "total" => $total]);
			
		} catch (\Exception $e) {
			// 详细的错误日志记录
			$errorLog = CMF_ROOT . 'data/debug_withdraw.log';
			$errorData = [
				'timestamp' => date('Y-m-d H:i:s'),
				'method' => 'affiwithdrawrecord',
			...
			file_put_contents($errorLog, "=== ERROR OCCURRED ===\n" . json_encode($errorData, JSON_UNESCAPED_UNICODE) . "\n", FILE_APPEND);
			
			// 返回详细错误信息而不是通用500错误
			return jsonrule([
				"status" => 500, 
				"msg" => "调试模式 - 错误详情: " . $e->getMessage(),
				"debug_info" => [
					"file" => $e->getFile(),
					"line" => $e->getLine(),
