...
class ReportsController extends AdminBaseController
{
	...
	public function rankForwardClient()
	{
		try {
			// 调试日志记录 - 记录请求开始
			$debugLog = CMF_ROOT . 'data/debug_forward_client.log';
			$logData = [
				'timestamp' => date('Y-m-d H:i:s'),
				'method' => 'rankForwardClient',
			...
			file_put_contents($debugLog, "Building main query for clients ranking...\n", FILE_APPEND);
			...
			file_put_contents($debugLog, "Main query executed successfully. Result count: " . count($clients) . "\n", FILE_APPEND);

			if ($clients) {
				file_put_contents($debugLog, "Processing clients data...\n", FILE_APPEND);
				...
				file_put_contents($debugLog, "Sorting clients by 'last' field...\n", FILE_APPEND);
				...
				file_put_contents($debugLog, "Data processing completed. Final result count: " . count($data) . "\n", FILE_APPEND);
			} else {
				file_put_contents($debugLog, "No clients data found.\n", FILE_APPEND);
				$data = [];
			}
			...
			// 详细的错误日志记录
			$errorLog = CMF_ROOT . 'data/debug_forward_client.log';
			$errorData = [
				'timestamp' => date('Y-m-d H:i:s'),
				'method' => 'rankForwardClient',
